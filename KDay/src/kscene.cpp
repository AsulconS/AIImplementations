...
void processImage(vector<vector<double>>& points, const std::string& path, double imclass)
{
    ...
    Image inputImg {path.c_str(), GRAY};
...
int main(int argc, char** argv)
{
    ...
    classmap[0.0].first = "Class 0: Buildings";
    classmap[1.0].first = "Class 1: Forest";
    classmap[2.0].first = "Class 2: Glacier";
    classmap[3.0].first = "Class 3: Mountain";
    classmap[4.0].first = "Class 4: Sea";
    classmap[5.0].first = "Class 5: Street";

    // Get Training Points
    std::cout << "Processing... " << std::endl;
    vector<vector<double>> trainPoints;
    for(const auto& folder : std::filesystem::directory_iterator("dataset/seg_train"))
    {
    ...
    for(const auto& folder : std::filesystem::directory_iterator("dataset/seg_test"))
    {
        for(const auto& entry : std::filesystem::directory_iterator(folder))
        {
            std::string fpath {folder.path().string()};
    ...
    processImage(predictions, std::string{"dataset/seg_pred/"} + argv[1], -1.0);
